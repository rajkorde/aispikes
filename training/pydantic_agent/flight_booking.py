...
class FlightDetails(BaseModel):
    ...
    price: int = Field(description="Price of the flight")
...
# Agent responsible for controlling the flow of the conversation
...
# Agent responsible for extracting flight details from web page text
...
async def validate_result(
    ...
    """Procedural validation that the flights meet the constraints."""
...
# Agent responsible for extracting user's seat preferences
...
async def find_seat(usage: Usage) -> SeatPreference:
    ...
    while True:
        ...
        if isinstance(result.data, SeatPreference):
            ...
            print("Coult not understand seat preference. Please try again.")
            message_history = result.all_messages()


async def buy_tickets(flight_details: FlightDetails, seat: SeatPreference) -> None:
    print(f"Purchasing tickets for {flight_details=!r} {seat=!r}")
...
async def main():
    ...
    while True:
        result = await search_agent.run(
            f"Find a flight from {deps.req_origin} to {deps.req_destination} on {deps.req_date}",
            deps=deps,
            message_history=message_history,
            usage=usage,
            usage_limits=usage_limits,
