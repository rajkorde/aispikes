...
config = load_configuration_file("config/podcast_downloader_config.json")
...
last_downloaded_file = (
    only_files_from_feed_sorted[-1] if only_files_from_feed_sorted else None
)

download_limiter_function = partial(
    build_only_new_entities(to_name_function),
    last_downloaded_file if last_downloaded_file else "",
)
